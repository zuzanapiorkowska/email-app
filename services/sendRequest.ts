...
export class SendRequest {
  ...
  async sendAnsweredSurvey(questionsWithAnswers: AnsweredQuestion[]): Promise<Confirmation> {
    const url = "http://localhost:3000/api/email/";
    try {
      console.log("tried to send q&a");
      const response = await axios.post(url, { questionsWithAnswears: questionsWithAnswers });
      const ConfirmationAnswer: Confirmation = response.data;
      return ConfirmationAnswer;
    } catch (err: any) {
      throw new Error(
